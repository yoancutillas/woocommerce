...
export const tagReleaseCommand = new Command( 'tag-release' )
	...
	.option(
		'-d, --dry-run',
		'Skips the pushing and tagging the release.'
	)
	.action( async ( project, version, { owner, targetRepo, targetBranch, dryRun } ) => {
		const projectPath = getProjectPathFromFilter( project );
		if ( ! projectPath ) {
			Logger.error( 'The project specified should match exactly one pnpm filter' );
		}
		...
		const tmpRepoPath = await cloneAuthenticatedRepo( { owner, name: repo }, false );
