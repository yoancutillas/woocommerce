...
export const AppLogin: FunctionComponent< AppLoginProps > = ( {
    ...
    console.log("state", {jetpackInstallState, onClickInstall, isBusy});
	...
	const JetpackStatus = () => {
		return (
			<div className="jetpack-container">
				...
				<div>
                    TODO You'll need several things before you get started
...
export const AppLoginWrapper = () => {
	...
	if ( ! canUserInstallPlugins ) {
		return null; // sorry dave we can't do anything without installing plugins
	}
	...
	const onClickInstall = () => {
        ...
        // TODO no user connected state is not represented
        // TODO: redirect talks about setting up store, might need to fix that?
        // Navigated to https://ends-amy-derived-artwork.trycloudflare.com/wp-admin/admin.php?page=jetpack&action=register&_wpnonce=1aa0f1253e&redirect=https%3A%2F%2Fends-amy-derived-artwork.trycloudflare.com%2Fwp-admin%2Fadmin.php%3Fpage%3Dwc-admin&from=woocommerce-onboarding&calypso_env=production
	};
    ...
    // if 'unavailable': sorry we can't do this dave
    // if 'installed': we need to connect jetpack?
    // if 'activated': check if isSettingUpJetpack and if user exists
    // if isSettingUpJetpack: show spinner and text -> 'connecting'
    // if !isSettingUpJetpack: 
    // and user connected show button to send magic link -> 'ready'
    // if user not connected we need to connect jetpack?
...
interface AppLoginProps {
	jetpackInstallState: 'activated' | 'installed' | 'unavailable' | 'connecting' | 'ready';
	isBusy: boolean;
	onClickInstall: () => void;
}
