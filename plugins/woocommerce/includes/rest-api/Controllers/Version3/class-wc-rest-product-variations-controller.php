...
class WC_REST_Product_Variations_Controller extends WC_REST_Product_Variations_V2_Controller {
	...
	public function prepare_object_for_response( $object, $request ) {
		...
		$data = array(
			...
			'description'           => wc_format_content( $object->get_description( $context ) ),
			'permalink'             => $object->get_permalink( $context ),
			'sku'                   => $object->get_sku( $context ),
			'price'                 => $object->get_price( $context ),
			'regular_price'         => $object->get_regular_price( $context ),
			'sale_price'            => $object->get_sale_price( $context ),
			...
			'purchasable'           => $object->is_purchasable( $context ),
			'virtual'               => $object->is_virtual( $context ),
			'downloadable'          => $object->is_downloadable( $context ),
			'downloads'             => $this->get_downloads( $object ),
			'download_limit'        => '' !== $object->get_download_limit( $context ) ? (int) $object->get_download_limit( $context ) : -1,
			'download_expiry'       => '' !== $object->get_download_expiry( $context ) ? (int) $object->get_download_expiry( $context ) : -1,
			'tax_status'            => $object->get_tax_status( $context ),
			'tax_class'             => $object->get_tax_class( $context ),
			'manage_stock'          => $object->managing_stock( $context ),
			...
			'backorders_allowed'    => $object->backorders_allowed( $context ),
			'backordered'           => $object->is_on_backorder( $context ),
			'low_stock_amount'      => '' === $object->get_low_stock_amount( $context ) ? null : $object->get_low_stock_amount( $context ),
